...
{
	// We use the Khronos texture format (https://www.khronos.org/opengles/sdk/agents/KTX/file_format_spec/)
	std::string filename = vkb::fs::path::get(vkb::fs::path::Assets, "textures/metalplate01_rgba.ktx");
	// ktx1 doesn't know whether the content is sRGB or linear, but most agents save in sRGB, so assume that.
